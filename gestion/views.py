    ...
    def post(self, request: Request | HttpRequest):
        ...
        try:
            ...
            return Response(data={
                ...
                # data > atributo que convierte la instancia a un diccionar utilizando tipos de datos complejos
...
class CategoriaController(APIView):
    def get(self, request: Request | HttpRequest, id: int):
        ...
        categoriaEncontrada = Categoria.objects.filter(
            id=id, habilitado=True).first()
    ...
    def put(self, request: Request | HttpRequest, id: int):
        ...
        try:
            ...
            serializador.update(instance=categoriaEncontrada,
                                validated_data=dataValidada)
...
class LibrosController(APIView):
    ...
    def get(self, request: Request | HttpRequest):
        ...
        if request.query_params.get('titulo'):
            parametros['titulo__icontains'] = request.query_params.get(
                'titulo')

        if request.query_params.get('anio'):
            # hace busqueda del anio de esa fecha
            parametros['fechaPublicacion__year'] = request.query_params.get(
                'anio')
